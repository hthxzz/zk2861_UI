<template>
    <b-hot @on_click="$_click"
           @on_dblclick="$_dblclick"
           @on_move="$emit('on_move', $event)"
           @on_enter="$emit('on_enter', $event)"
           @on_leave="$emit('on_leave', $event)"
           :styles="`rel mrg-t-1 mrg-r-1 pad-v-.4 pad-h-.5 round-sm line thick-1 line-${tag_colors.normal.line} color-${tag_colors.normal.text} bg-color-${tag_colors.normal.bg} w-${minWidth} flex-${headIcon? '4': '5'}`"
           :states="{
               act: {
        ...
        <!--项目图标-->
        <b-icon v-if="headIcon" styles="bg-size-contain bg-pos-5 mrg-r-.5 w-1.4 h-1.4 flex-5" v-bind="headIcon" />
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //双击操作
            $_dblclick(){
                this.$emit("on_dblclick", this.id, state, e);
            }
